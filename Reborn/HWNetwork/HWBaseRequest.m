...
NSString *const HWRequestValidationErrorDomain = @"request.validation";

@interface HWBaseRequest ()
...
- (void)setCompletionBlockWithSuccess:(HWRequestCompletionBlock)success
                              failure:(HWRequestCompletionBlock)failure {
...
- (void)start {
    ...
    [[HWNetworkAgent sharedAgent] addRequest:self];
}

- (void)stop {
    [self toggleAccessoriesWillStopCallBack];
    self.delegate = nil;
    [[HWNetworkAgent sharedAgent] cancelRequest:self];
    [self toggleAccessoriesDidStopCallBack];
}

- (void)startWithCompletionBlockWithSuccess:(HWRequestCompletionBlock)success
                                    failure:(HWRequestCompletionBlock)failure {
...
- (HWRequestSerializerType)requestSerializerType {
    return HWRequestSerializerTypeHTTP;
}

- (HWResponseSerializerType)responseSerializerType {
    return HWResponseSerializerTypeJSON;
}
