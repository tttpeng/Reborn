...
void HWLog(NSString *format, ...) {
#ifdef DEBUG
    if (![HWNetworkConfig sharedConfig].debugLogEnabled) {
        return;
    }
    va_list argptr;
...
+ (void)addDoNotBackupAttribute:(NSString *)path {
    ...
    if (error) {
        HWLog(@"error to set do not backup attribute, error = %@", error);
    }
}
...
@implementation HWBaseRequest (RequestAccessory)

- (void)toggleAccessoriesWillStartCallBack {
    for (id<HWRequestAccessory> accessory in self.requestAccessories) {
...
@implementation HWBatchRequest (RequestAccessory)
...
@implementation HWChainRequest (RequestAccessory)
